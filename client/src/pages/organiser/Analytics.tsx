...
export default function Analytics() {
  ...
  const exportAsExcel = async () => {
    ...
    const titleFont: Partial<ExcelJS.Font> = {
      bold: true, color: { argb: 'FF4F46E5' }, size: 14
    };
    ...
    [
      ...
      ['Active Events', data.performance.length],
    ].forEach(([label, value]) => {
    ...
    [
      ['Event Name', data.top?.name ?? 'N/A'],
      ['Attendees', data.top?.sub ?? 'N/A'],
      ['Check-in Rate', data.top ? `${data.top.progress}%` : 'N/A'],
    ].forEach(([label, value]) => {
      const row = summary.addRow([label, value]);
      row.eachCell(cell => { cell.border = border; });
      row.getCell(1).font = { color: { argb: 'FF6B7280' }, size: 10 };
    ...
    perf.columns = [
      { width: 8 }, { width: 35 }, { width: 18 }, { width: 20 }, { width: 20 }
    ];
    ...
    monthlyColHeader.eachCell(cell => {
      cell.fill = headerFill;
      cell.font = headerFont;
      cell.border = border;
      cell.alignment = { horizontal: 'center' };
    ...
    insightsColHeader.eachCell(cell => {
      cell.fill = headerFill;
      cell.font = headerFont;
      cell.border = border;
    });
    ...
    }), `EventsHub_Analytics_${today}.xlsx`);
  };
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Legend
                      verticalAlign="top"
                      align="right"
                      wrapperStyle={{ fontSize: '12px', paddingBottom: '16px', fontWeight: 500 }}
                      iconType="circle"
                    />
                    <Bar dataKey="value" name="Attendance" radius={[6, 6, 0, 0]} maxBarSize={45}>
                      {data.attendanceData.map((_, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={index === data.attendanceData.length - 1 ? '#6366f1' : '#818cf8'}
                        />
                      ))}
                    </Bar>
                  </BarChart>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
              ...
              <div className="space-y-4">
                <InsightItem
                  label="Top Event"
                  value={data.top ? `${data.top.name} had the highest attendance.` : "No events yet."}
                />
                <InsightItem
                  label="Attendance Rate"
                  value={`${data.attendanceRate}% of registered attendees checked in.`}
                />
                <InsightItem
                  label="Total Reach"
                  value={`${data.totalAttendees.toLocaleString()} attendees across all events.`}
                />
              </div>
            </div>
